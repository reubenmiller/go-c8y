...
func TestListPlucksReferences(t *testing.T) {
	...
	if path != "/inventory/managedObjects/100/childDevices" {
...
// TestGetUsesChildDevicesPath verifies GET …/{child} hits the childDevices path
// (regression guard: the endpoint constant previously pointed at childAdditions)
// and returns the nested managedObject of the reference.
func TestGetUsesChildDevicesPath(t *testing.T) {
	var path string
	svc, closeFn := testService(func(w http.ResponseWriter, r *http.Request) {
		path = r.URL.Path
		w.Header().Set("Content-Type", "application/json")
	...
	if path != "/inventory/managedObjects/100/childDevices/77" {
		t.Errorf("get path = %q, want .../childDevices/77", path)
	}
	if res.Data.ID() != "77" {
...
func TestCreatePostsManagedObject(t *testing.T) {
	...
	if method != http.MethodPost || path != "/inventory/managedObjects/100/childDevices" {
...
func TestUnassignDeletesByPath(t *testing.T) {
	...
	if method != http.MethodDelete || path != "/inventory/managedObjects/100/childDevices/55" {
		t.Errorf("unassign = %s %s, want DELETE .../childDevices/55", method, path)
	}
	if bodyLen != 0 {
