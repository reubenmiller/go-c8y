...
func (n *getApplicationCollectionCmd) getApplicationCollection(cmd *cobra.Command, args []string) error {
	...
	return n.doGetApplicationCollection("GET", path, queryValue, body.GetMap(), filters)
}

func (n *getApplicationCollectionCmd) doGetApplicationCollection(method string, path string, query string, body map[string]interface{}, filters *JSONFilters) error {
	...
	if resp != nil && resp.JSONData != nil {
		...
		if filters != nil && !globalFlagRaw {
			responseText = filters.Apply(*resp.JSONData, "applications")
		} else {
