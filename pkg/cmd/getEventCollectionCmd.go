...
func (n *getEventCollectionCmd) getEventCollection(cmd *cobra.Command, args []string) error {
	...
	return n.doGetEventCollection("GET", path, queryValue, body.GetMap(), filters)
}

func (n *getEventCollectionCmd) doGetEventCollection(method string, path string, query string, body map[string]interface{}, filters *JSONFilters) error {
	...
	if resp != nil && resp.JSONData != nil {
		...
		if filters != nil && !globalFlagRaw {
			responseText = filters.Apply(*resp.JSONData, "events")
		} else {
