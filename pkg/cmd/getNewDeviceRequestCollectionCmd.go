...
func (n *getNewDeviceRequestCollectionCmd) getNewDeviceRequestCollection(cmd *cobra.Command, args []string) error {
	...
	return n.doGetNewDeviceRequestCollection(req, outputfile, filters)
}

func (n *getNewDeviceRequestCollectionCmd) doGetNewDeviceRequestCollection(req c8y.RequestOptions, outputfile string, filters *JSONFilters) error {
	...
	if resp != nil && resp.JSONData != nil {
		...
		if isJSONResponse && filters != nil && !globalFlagRaw {
			responseText = filters.Apply(*resp.JSONData, "newDeviceRequests")
		} else {
