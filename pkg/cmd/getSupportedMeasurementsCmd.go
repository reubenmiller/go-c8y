...
func (n *getSupportedMeasurementsCmd) getSupportedMeasurements(cmd *cobra.Command, args []string) error {
	...
	return n.doGetSupportedMeasurements(req, outputfile, filters)
}

func (n *getSupportedMeasurementsCmd) doGetSupportedMeasurements(req c8y.RequestOptions, outputfile string, filters *JSONFilters) error {
	...
	if resp != nil && resp.JSONData != nil {
		...
		if isJSONResponse && filters != nil && !globalFlagRaw {
			responseText = filters.Apply(*resp.JSONData, "c8y_SupportedMeasurements")
		} else {
