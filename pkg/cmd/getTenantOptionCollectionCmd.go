...
func (n *getTenantOptionCollectionCmd) getTenantOptionCollection(cmd *cobra.Command, args []string) error {
	...
	return n.doGetTenantOptionCollection("GET", path, queryValue, body.GetMap(), filters)
}

func (n *getTenantOptionCollectionCmd) doGetTenantOptionCollection(method string, path string, query string, body map[string]interface{}, filters *JSONFilters) error {
	...
	if resp != nil && resp.JSONData != nil {
		...
		if filters != nil && !globalFlagRaw {
			responseText = filters.Apply(*resp.JSONData, "options")
		} else {
