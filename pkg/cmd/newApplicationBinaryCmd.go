...
func (n *newApplicationBinaryCmd) newApplicationBinary(cmd *cobra.Command, args []string) error {
	...
	return n.doNewApplicationBinary("POST", path, queryValue, body.GetMap(), filters)
}

func (n *newApplicationBinaryCmd) doNewApplicationBinary(method string, path string, query string, body map[string]interface{}, filters *JSONFilters) error {
